<template>
  <div class="flex h-screen bg-[#FFF8F4] overflow-hidden -m-6">
      ...
      <div class="flex-1 overflow-y-auto p-5">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
          <button v-for="p in filteredProducts" :key="p.id" @click="addToCart(p)"
            class="bg-white rounded-2xl border border-[#EDE8E3] p-3 text-left transition-all hover:border-[#E8634A]/40 hover:shadow-md hover:-translate-y-0.5 active:scale-95 group">
            <div class="w-full aspect-square rounded-xl bg-[#FFF0EC] flex items-center justify-center text-4xl mb-3 group-hover:bg-[#FFE4DC] transition-colors">
              {{ p.emoji }}
            </div>
            <div class="text-xs font-black text-[#1E2A3B] truncate mb-1">{{ p.name }}</div>
            <div class="text-xs font-bold text-[#E8634A]">{{ formatPrice(p.price) }}</div>
          </button>
        </div>
      </div>
    </div>
    ...
    <div class="w-[360px] shrink-0 bg-white flex flex-col">
      <div class="px-5 py-4 border-b border-[#EDE8E3] flex items-center justify-between shrink-0">
        <div>
      ...
      <div class="flex-1 overflow-y-auto px-4 py-3">
        ...
        <div v-else class="space-y-2">
          <div v-for="item in cart" :key="item.id"
            ...
            <div class="w-10 h-10 rounded-lg bg-[#FFF0EC] flex items-center justify-center text-xl shrink-0">{{ item.emoji }}</div>
            <div class="flex-1 min-w-0">
              <div class="text-xs font-bold text-[#1E2A3B] truncate">{{ item.name }}</div>
              <div class="text-xs text-[#E8634A] font-bold">{{ formatPrice(item.price) }}</div>
            </div>
            <div class="flex items-center gap-1.5 shrink-0">
              ...
              <button @click="increase(item)" class="w-7 h-7 rounded-lg bg-[#E8634A] flex items-center justify-center text-white hover:bg-[#d4583f] transition font-bold text-lg leading-none">+</button>
            </div>
      ...
      <div class="border-t border-[#EDE8E3] px-5 pt-4 pb-3 shrink-0 space-y-2">
        ...
        <div class="flex justify-between text-sm text-gray-500">
          <span>VAT (8%)</span>
          <span class="font-semibold text-[#1E2A3B]">{{ formatPrice(vat) }}</span>
        </div>
        <div class="flex justify-between text-base font-black text-[#1E2A3B] pt-2 border-t border-[#EDE8E3]">
          <span>Tong cong</span>
          <span class="text-[#E8634A] text-lg">{{ formatPrice(total) }}</span>
        </div>
      </div>
      ...
      <div class="px-5 pb-5 space-y-2 shrink-0">
        <button @click="openPayment('cash')" :disabled="cart.length === 0"
          ...
          :class="cart.length === 0 ? 'bg-gray-100 text-gray-300 cursor-not-allowed' : 'bg-gradient-to-r from-[#E8634A] to-[#F07A5E] text-white shadow-lg shadow-[#E8634A]/30 hover:-translate-y-0.5'">
        ...
        <button @click="openPayment('qr')" :disabled="cart.length === 0"
          ...
          :class="cart.length === 0 ? 'border-gray-200 text-gray-300 cursor-not-allowed' : 'border-[#E8634A] text-[#E8634A] hover:bg-[#FFF0EC]'">
          <iconify-icon icon="ph:qr-code-duotone" class="text-xl"></iconify-icon>
          VietQR / Chuyen khoan
        </button>
        <button @click="printBill" :disabled="cart.length === 0"
          class="w-full py-3 rounded-xl font-black text-sm flex items-center justify-center gap-2 transition-all"
          :class="cart.length === 0 ? 'bg-gray-100 text-gray-300 cursor-not-allowed' : 'bg-[#1E2A3B] text-white hover:bg-[#2D3F56]'">
    ...
    <el-dialog v-model="showCashModal" title="Thanh toan tien mat" width="400px" center align-center>
      <div class="space-y-4">
        <div class="text-center p-4 bg-[#FFF8F4] rounded-xl">
          ...
          <div class="text-3xl font-black text-[#E8634A]">{{ formatPrice(total) }}</div>
        </div>
        <div>
          <label class="text-sm font-bold text-[#1E2A3B] mb-2 block">Tien khach dua</label>
    ...
    <el-dialog v-model="showQRModal" title="Thanh toan VietQR" width="380px" center align-center>
      <div class="flex flex-col items-center gap-4">
        <div class="w-52 h-52 bg-[#1E2A3B] rounded-2xl flex items-center justify-center p-4">
        ...
        <div class="text-center p-3 bg-[#FFF8F4] rounded-xl w-full">
          <div class="text-2xl font-black text-[#E8634A]">{{ formatPrice(total) }}</div>
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="confirmPayment" style="background-color: #E8634A; border-color: #E8634A;">
    ...
    <el-dialog v-model="showSuccessModal" width="360px" center align-center :show-close="false">
      <div class="flex flex-col items-center gap-3 py-4">
        ...
        <div class="text-xl font-black text-[#1E2A3B]">Thanh toan thanh cong!</div>
...
const allProducts = [
  { id: 1,  name: 'Banh sinh nhat 3D',    emoji: '🎂', price: 350000,  category: 'banh' },
  { id: 2,  name: 'Banh mousse chanh leo', emoji: '🍰', price: 120000,  category: 'banh' },
  { id: 3,  name: 'Cupcake dau tay',       emoji: '🧁', price: 35000,   category: 'banh' },
  { id: 4,  name: 'Banh cuoi cao cap',     emoji: '💍', price: 1500000, category: 'banh' },
  { id: 5,  name: 'Macaron hon hop',       emoji: '🍬', price: 25000,   category: 'banh' },
  { id: 6,  name: 'Banh tiramisu',         emoji: '☕', price: 95000,   category: 'banh' },
  { id: 7,  name: 'Banh red velvet',       emoji: '❤️', price: 280000,  category: 'banh' },
  { id: 8,  name: 'Banh flan caramen',     emoji: '🍮', price: 45000,   category: 'banh' },
  { id: 9,  name: 'Banh su kem',           emoji: '🥐', price: 20000,   category: 'banh' },
  { id: 10, name: 'Croissant bo',          emoji: '🥖', price: 30000,   category: 'banh' },
  { id: 11, name: 'Nen sinh nhat (set)',   emoji: '🕯️', price: 15000,   category: 'phukien' },
  { id: 12, name: 'Hop banh cao cap',      emoji: '📦', price: 25000,   category: 'phukien' },
  { id: 13, name: 'Ruy bang trang tri',    emoji: '🎀', price: 10000,   category: 'phukien' },
  { id: 14, name: 'Tui giay Polycake',     emoji: '🛍️', price: 8000,    category: 'phukien' },
  { id: 15, name: 'The loi chuc',          emoji: '💌', price: 5000,    category: 'phukien' },
]
  ...
  const html = `
    ...
    <body>
      ...
      <div style="text-align:center; margin-top:8px; font-size:10px;">
        ...
        <div>Hen gap lai tai Polycake!</div>
